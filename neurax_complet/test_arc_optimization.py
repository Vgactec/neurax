...
class OptimizationTester:
    ...
    def __init__(self, data_path="arc_data"):
        ...
        if not os.path.exists(self.data_path):
            ...
            logger.info(f"Dossier de données créé: {self.data_path}")

        if not self._verify_data_files():
            logger.error("Initialisation impossible - fichiers manquants")
            self.arc_data = None
        else:
        ...
        if self.arc_data:
            ...
            logger.warning("ARC data could not be loaded.")
